...
def get_questions_for_explanations():
    query_constructor = create_query(
        select=['questions.question_id', 'questions.question_text', ' answers.answer_text'],
        from_table=['questions'],
        join=[('answers', "answers.question_id = questions.question_id", "INNER JOIN")],
        where=[("answers.answer_type", "right")]
    )
    query_constructor.where_raw("questions.answer_explanations IS NULL")
    questions_list = get_query_results(query_constructor)
    return questions_list


def get_explanations_update_db(questions_list):
    for question_id, question_text, answer_text in questions_list:
        inp_str = {
            'question': question_text,
            'answer': answer_text
        }

        print(question_id, question_text, answer_text, "\n\n\n")
        ...
        query_constructor = update_table_query('questions', ("answer_explanations", answer_explanation), ("question_id", question_id))
